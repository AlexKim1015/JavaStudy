...
public class ArrayMain {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException {
		...
		// 참조타입 특정 메모리에 담겨있는 주소값을 가지고 있는 애들 (String, Object..) : null
		// 기본타입 값 그 자체가 담겨있는 애들 (int, boolean..) : 숫자이면 0, boolean 이면 false
		...
		printArr(seoyugi);
		// 오공이 포함되어있는 배열의 요소를 찾아서 서유기를 -> 드래곤볼 로 변경
		for(int i = 0; i < seoyugi.length; i ++) {
...
//		String [] students =new String[] {"팽수","길동","동길","길수"}; //위와 동일함.
		...
		for(int i =0 ; i < subArr.length ; i ++) {
			subArr[i]= subArr[i].trim(); // trim <-- 양족공백제거 
		}
		printArr(subArr);
		
		// 배열의 복사1.얇은 복시? shallow copy 주소값 복사
